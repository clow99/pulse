(function () {
  ...
  var collectWebVitals = script.getAttribute('data-web-vitals') === 'true';
  var VISIT_TTL = 30 * 60 * 1000;
  var VISIT_KEY = 'pulse:visit';
  ...
  function sendVital(name, value) {
    if (!collectWebVitals || !Number.isFinite(value)) return;
  ...
  function observeVitals() {
    if (!collectWebVitals || typeof PerformanceObserver !== 'function') return;
    ...
    function flushFinalVitals() {
      if (lcp && sentVitals.LCP !== lcp) sendVital('LCP', lcp);
      if (cls && sentVitals.CLS !== cls) sendVital('CLS', cls);
      if (inp && sentVitals.INP !== inp) sendVital('INP', inp);
    }
